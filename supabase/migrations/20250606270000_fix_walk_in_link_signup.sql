-- Signup failed when handle_new_user called link_walk_in_projects_to_client inside the
-- auth.users trigger (auth.uid() is null → RLS blocks booking writes → whole signup rolls back).
...
begin
  -- Bypass RLS: this runs as a trusted server-side link (trigger has no auth.uid()).
  ...
  if public.normalize_client_name(v_profile.full_name) = '' then
    return 0;
  end if;
  ...
  for v_project in
    ...
    where public.normalize_client_name(p.artist_name) = public.normalize_client_name(v_profile.full_name)
...
-- Do not link inside the auth trigger; the app calls the RPC after signup/login instead.
